...
final class UpdateTransaction
{
    ...
    public function handle(User $user, int $id, array $data): Transaction
    {
        return DB::transaction(fn (): Transaction => $this->writer->fill($user, $user->transactions()->findOrFail($id), $data));
    }
}
