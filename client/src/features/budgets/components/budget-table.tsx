...
export function BudgetTable({ budgets, onEdit, onDelete }: BudgetTableProps) {
  return (
    <div className="border">
      <Table>
        <TableHeader><TableRow><TableHead>Expense category</TableHead><TableHead>Planned amount</TableHead><TableHead className="w-28 text-right">Actions</TableHead></TableRow></TableHeader>
        <TableBody>{budgets.map((budget) => {
          ...
          return <TableRow key={budget.id}>
            ...
            <TableCell className="font-mono font-medium">{formatPhp(budget.amount)}</TableCell>
